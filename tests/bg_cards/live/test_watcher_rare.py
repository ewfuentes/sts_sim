...
def test_ragnarok_with_strength(game):
...
def test_ragnarok_upgraded(game):
...
def test_brilliance_basic(game):
...
def test_blasphemy_exhaust(game):
...
def test_deus_ex_machina_basic(game):
...
def test_scrawl_draw_exhaust(game):
...
def test_spirit_shield_basic(game):
...
def test_judgment_above_threshold(game):
...
def test_establishment_play(game):
...
def test_conjure_blade_x_cost(game):
